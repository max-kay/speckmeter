...
pub fn search_minimum<P>(problem: P, initial_params: Vec<f32>, max_iterations: u32) -> Vec<f32>
where
    P: Gradient + Cost,
{
    ...
    let c = 0.5; // e (0, 1)
    let tau = 0.8; //e (0, 1)
    let mut last_gamma = 20.0; // this value should be better determined TODO
    ...
    for i in 0..max_iterations {
        ...
        let gradient = dbg!(problem.gradient(parameters.clone()));
        let t = -c * inner_product(gradient.clone(), gradient.clone());
        let mut current_alpha = last_gamma;
        let gamma = loop {
        ...
        parameters = add(parameters, scale(gradient.clone(), -gamma));
        last_gamma = gamma;
    }
    parameters
}
...
fn scale(x: Vec<f32>, factor: f32) -> Vec<f32> {
